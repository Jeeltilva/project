...
export class CaseService {
  ...
  addCase(addcase: Addcase) {
    this.http
      .post<Addcase>(apiUrl + 'addcase', addcase)
  ...
  getpreadmittedCases() {
    this.http
      .get<Case[]>(apiUrl + 'preadmittedcases')
      .pipe(
        map((data: any) => {
          if (data) {
            return data;
  ...
  updateDetails(caseData) {
    this.http
      .patch<Case>(apiUrl + 'case/' + caseData._id, {
  ...
  updateCase(caseData) {
    this.http
      .patch<Case>(apiUrl + 'editcase/' + caseData._id, caseData)
      .pipe(catchError(this.handleError))
      .subscribe(
        (data: Case) => {
          this.singleCase.next(data);
  ...
  disposeCase(_id) {
    this.http
      .patch<any>(apiUrl + 'makedispose', { _id })
      .pipe(catchError(this.handleError))
      .subscribe((data) => {});
  }
  ...
  addOrder(id, data: any) {
    this.http
      .patch<Case>(`${apiUrl}case/order/${id}`, { order: data })
      .subscribe(
        (CaseData: Case) => {
  ...
  onUploadDoc(file: FileList, id) {
    ...
    return this.http
      .post<any>(apiUrl + 'uploadPdf/' + id, formData)
      .pipe(catchError(this.handleError));
  }

  onDeleteDoc(files, id) {
    this.http.patch<Case>(apiUrl + 'updateDoc/' + id, files).subscribe(
      (data: Case) => {
        this.singleCase.next(data);
      },
      (err: any) => {
  ...
  linkClient(_id, id) {
    this.http
      .patch<Case>(`${apiUrl}linkClient/${id}`, { userId: _id })
      .pipe(catchError(this.handleError))
      .subscribe(
        (data: Case) => {
          this.singleCase.next(data);
  ...
  getLinkedClient(id) {
    this.http
      .get<Client>(`${apiUrl}getLinkedClient/${id}`)
      .pipe(catchError(this.handleError))
      .subscribe(
        (data: Client) => {
          this.linkedClient.next(data);
  ...
  getLinkedLawyer(id) {
    this.http
      .get<LawyerInt>(`${apiUrl}getLinkedLawyer/${id}`)
      .pipe(catchError(this.handleError))
      .subscribe(
        (data: LawyerInt) => {
  ...
  getClient(id) {
    this.http
      .get<Client>(`${apiUrl}getClient/${id}`)
      .pipe(catchError(this.handleError))
      .subscribe(
        (data: Client) => {
          this.linkedClient.next(data);
  ...
  searchclients(term): Observable<Client[]> {
    ...
    return this.http
      .get<Client[]>(`${apiUrl}getClients?firstname=${term}`)
      .pipe(
        tap((x) => console.log(x)),
  ...
  deletePDF(filename) {
    this.http
      .delete<any>(apiUrl + 'deletefile/' + filename)
      .pipe(catchError(this.handleError))
      .subscribe((data) => {
        // alert(data.message);
  ...
  getClientCases() {
    this.http
      .get<Case[]>(apiUrl + 'getClientCases')
      .pipe(
        map((data: any) => {
          if (data) {
            return data;
  ...
  getLawbook() {
    return this.http.get<any>(apiUrl + 'getLawbook');
  }
}
