...
const Lawbook = require("../Schema/lawbook");
...
router.delete("/api/case/:id", auth, async (req, res) => {
  ...
  try {
    ...
    caseData.docs.forEach((e) =>
      gfs.remove({ filename: e.filename }, function (err) {
        if (err) {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      res
        .status(404)
        .json({ message: "Data not found", errors: e.stack, body: req.body });
    }
  }
);
...
router.get("/api/getLawbook", async (req, res) => {
  try {
    const data = await Lawbook.find({});
    if (!data) {
      res.status(201).send([]);
    }
    res.status(201).send(data);
